...
DadGUI::cBypassOnOffManager __BypassOnOffManager;
...
DadGUI::GUI_EventManager __GUI_EventManager;

// System state variables
volatile eOnOff __OnOffCmd  = eOnOff::ByPass; // Pending state change command
volatile eOnOff __MemOnOff  = eOnOff::ByPass; // Current audio processing state
volatile uint32_t __CT = 0;                   // Activity counter for LED heartbeat
...
//**********************************************************************************
static bool MidiFifoPush(const MidiEvent_t* event) {
...
void HardwareAndCoInitialize() {
    ...
    __BypassOnOffManager.Initialize((volatile uint8_t*)& __MemOnOff);
...
void AudioCallback(AudioBuffer *pIn, AudioBuffer *pOut) {
    ...
    for (size_t i = 0; i < AUDIO_BUFFER_SIZE; i++) {
        ...
        __GUI_EventManager.sendEventToActive_RT_ProcessIn(pIn);
        __Effect.Process(pIn, pOut, __MemOnOff, Silence);
        __GUI_EventManager.sendEventToActive_RT_Process();
        __GUI_EventManager.sendEventToActive_RT_ProcessOut(pOut);
...
void MainLoop() {
	...
	uint32_t GUIFastTrigger = HAL_GetTick();  // GUI fast update timer
    ...
    while (1) {
        ...
        // Update fast GUI process at regular intervals
        if ((CurrentTick - GUIFastTrigger) >= GUI_FAST_UPDATE_MS) {
            GUIFastTrigger = CurrentTick;
            __GUI_EventManager.sendEventToActive_FastUpdate();  // Update fast GUI process
        }

        // Update GUI at regular intervals
        if ((CurrentTick - GUITrigger) >= GUI_UPDATE_MS) {
            GUITrigger = CurrentTick;
            __GUI_EventManager.sendEventToActive_Update();   // Update GUI process
